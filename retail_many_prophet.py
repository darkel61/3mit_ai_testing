...
for index, product in enumerate(products):
    ...
    try:
        ...
        model_components = ModelComponentsParam(
            # regressors={
            #     "regressor_cols": ["sale_price"]  # Si tienes variables adicionales como "sale_price"
            # },
            # events=events,  # Los eventos como días festivos
            growth={
                "growth_term": "linear"  # Puede ser "linear" o "logistic" para Prophet
            },
            seasonality={
                "yearly_seasonality": 10,  # Ajusta según lo necesario
                "weekly_seasonality": True,
            },
            # changepoints={
            #     "changepoints_dict": dict(
            #         method="auto",  # Puede ser "auto" o definir manualmente los changepoints
            ...
            custom={
                "fit_algorithm_dict": {
                    "model_template": "PROPHET"  # Especifica que el modelo es Prophet
                }
            }
        )

        # Corre el pronóstico con Prophet
        result = forecaster.run_forecast_config(
            df=df,
            config=ForecastConfig(
                model_template="PROPHET",  # Cambiar a Prophet
                forecast_horizon=35,  # Predice 35 días hacia adelante
                coverage=0.90,  # Intervalo de confianza
        ...
        fig = forecast.plot()
        fig.write_html(f"html/{products_name[index]}-{product}-prophet.html")

        print(f"html/{products_name[index]}-{product}-prophet.html")
